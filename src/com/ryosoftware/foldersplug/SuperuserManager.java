	...
	public boolean beginSession()
	{
		if (iProcess == null)
		{
			...
			try
			{
				...
				if (isRoot()) iCanRunRootCommands = correct = true;
			}
			catch (Exception e)
			{
				closeSession();
		...
		try
		{
			if (iProcess != null) 
			{
				try
				{
					...
					if (uid == null) Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Can't get root access or denied by user");
					else 
					{
						if (! uid.contains("uid=0")) Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Root access rejected. I'm " + uid);
						else
						{
							is_root = true;
			...
			if (iProcess != null) 
			{
				if ((commands != null) && (commands.size() > 0))
				{
					try
					{
						getStandardOutput();
						getErrorOutput();
						for (int i = 0; i < AUTO_ADDED_EXECUTE_COMMANDS.length; i ++) commands.add(AUTO_ADDED_EXECUTE_COMMANDS [i]);
