...
public class SuperuserCommandsExecutor
{
    ...
    private boolean initializeSuperuserFields()
    {
        if ((! iLsFieldsInitializated) && (iSuperuserManager.canRunRootCommands()))
        {
            ...
            if (data != null)
            {
                ...
                else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "D: " + data.toString());
            }
            else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Error getting data output");
        }
    ...
    public ArrayList<String> getChildFolders(String pathname)
    {
        ...
        if ((initializeSuperuserFields()) && (canRunRootCommands()))
        {
            ...
            if (childs != null)
            {
                for (int i = childs.size() - 1; i >= 0; i --)
                {
                    ...
                    if ((iLsPrefixed) && (filename.startsWith(pathname + "/"))) filename = filename.substring(pathname.length());
        ...
        else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, iLsFieldsInitializated ? "Can't get root privileges" : "LS fields not initializated");
        return null;
    }
    ...
    public boolean isFolder(String pathname)
    {
        ...
        if ((initializeSuperuserFields()) && (canRunRootCommands()))
        {
            ...
            if (data != null)
            {
                if ((data.size() == 1) && (data.get(0).startsWith("d"))) is_folder = true;
                else if (data.size() != 1) Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Unparseable data output: " + data.toString());
            }
            else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Error getting data output");
        }
        else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, iLsFieldsInitializated ? "Can't get root privileges" : "LS fields not initializated");
    ...
    public boolean isEmptyFolder(String pathname)
    {
        ...
        if ((initializeSuperuserFields()) && (canRunRootCommands()))
        {
            ...
            else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Isn't a folder");
        }
        else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, iLsFieldsInitializated ? "Can't get root privileges" : "LS fields not initializated");
    ...
    public boolean moveFolderContents(String source, String destination)
    {
        ...
        {
            ArrayList<String> commands = new ArrayList<String>();
            for (int i = 0; i < RECURSIVE_MOVE_COMMAND.length; i ++) commands.add(String.format(RECURSIVE_MOVE_COMMAND [i], source, destination));
            iSuperuserManager.execute(commands);
    ...
    public boolean mountFolders(String source, String target)
    {
        ...
        if ((initializeSuperuserFields()) && (canRunRootCommands()))
        {
            ...
            if (data != null)
            {
                if (data.size() == 0) mounted = true;
                else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, data.toString());
            }
            else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, "Error getting data/errors output");
        }
        else Utilities.log(Constants.LOG_TITLE, LOG_SUBTITLE, iLsFieldsInitializated ? "Can't get root privileges" : "LS fields not initializated");
