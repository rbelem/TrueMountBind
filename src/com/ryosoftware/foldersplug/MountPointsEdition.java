...
public class MountPointsEdition extends Activity 
{
	...
	private class MountPointsListAdapter extends ArrayAdapter<MountPoint> implements OnClickListener, OnCreateContextMenuListener
	{
		...
		public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menu_info) 
		{
			...
			if (mountpoint != null)
			{
				...
				if (mountpoint.getEnabled())
				{
					if (mountpoint.getMounted()) menu.add(ContextMenu.NONE, UMOUNT_MOUNTPOINT_MENUITEM, ContextMenu.NONE, R.string.unmount_mountpoint_menuitem);
					else menu.add(ContextMenu.NONE, MOUNT_MOUNTPOINT_MENUITEM, ContextMenu.NONE, R.string.mount_mountpoint_menuitem);
				}
		...
		private class MountPointRemoveConfirmationCallback implements ButtonClickCallback
		{
			...
			public void onClick() 
			{
				...
				if (mountpoint != null) MainService.removeMountPoint (iActivity, iId);	
			}
		};
		...
		private class MountPointDeleteConfirmationCallback implements ButtonClickCallback
		{
			...
			{
				MountPoint mountpoint = getMountPointByIdentifier(iId);
				if (mountpoint != null) MainService.removeMountPoint (iActivity, mountpoint.getId());	
			}
		}
	...
	{
		if (intent.getBooleanExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_IS_FIRST_LIST_ELEMENT, false)) iMountPoints.clear();
		if (intent.getBooleanExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_HAS_VALUES, false)) iMountPoints.add(new MountPoint(intent.getIntExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_IDENTIFIER, -1), intent.getStringExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_SOURCE), intent.getStringExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_TARGET), intent.getBooleanExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_IS_ENABLED, false), intent.getBooleanExtra(MainService.ACTION_GET_MOUNT_STATES_ANSWER_IS_MOUNTED, false)));
    ...
    public boolean onPrepareOptionsMenu(Menu menu)
    {
    	...
    	if (enable_delete) for (int i = 0; i < iMountPoints.size(); i ++) if (iMountPoints.get(i).getMounted()) enable_delete = false;
    ...
    private boolean isMountPointEnableable(int id, String target)
    {
		...
		for (int i = 0; i < iMountPoints.size(); i ++)
		{
			...
			{
				if (mountpoint.getEnabled()) return true;
			}
			else if ((mountpoint.getTarget().equals(target)) && (mountpoint.getEnabled())) return false;
		}
    	return true;
    }
