...
public class ProcessUtilities 
{
	public static ArrayList<String> getForegroundApplications(Context context) 
	{
		...
		try
		{
			...
			for (RunningAppProcessInfo process : activity_manager.getRunningAppProcesses()) if (process.importance == RunningAppProcessInfo.IMPORTANCE_FOREGROUND) list.add(process.processName);
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public static boolean processRunning(String process_name)
	{
		...
		try
		{
			...
			if (os_in != null) os_in.close();
			if (os_out != null) os_out.close();
		}
		catch (Exception e)
		{
			e.printStackTrace();
	...
	public static boolean serviceRunning(String class_name, Context context)
	{
		...
		{
			ActivityManager activity_manager = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
			for (RunningServiceInfo service : activity_manager.getRunningServices(Integer.MAX_VALUE)) if (service.service.getClassName().equals(class_name)) return true;
		}
		catch (Exception e)
		{
			e.printStackTrace();
