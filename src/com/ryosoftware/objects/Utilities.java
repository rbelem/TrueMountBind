...
{	
	...
	{
		if (type == GET_STRING_FROM_BOOLEAN_TYPE_IS_AVAILABILITY) return value ? "enabled" : "disabled";
		if (type == GET_STRING_FROM_BOOLEAN_TYPE_IS_STATE) return value ? "ok" : "not ok";
	...
	public static ArrayList<String> convertVectorToList(String [] vector)
	{
		...
		for (int i = 0; i < vector.length; i ++) list.add(vector [i]);
		return list;
	}
